...
overview: DramLog의 향후 추가될 기능들을 순서대로 개발하는 계획입니다. Supabase Anonymous Auth와 RLS 설정을 최우선으로 진행합니다.
todos: []
---
...
- [ ] Supabase 대시보드 → SQL Editor에서 `supabase-schema.sql`의 RLS 정책 실행
...
1. 🔥 **Supabase Anonymous Auth + RLS 설정** (최우선)
2. 이미지 업로드 기능
3. 사용자 프로필 페이지
4. 북마크 기능
5. 태그 시스템
