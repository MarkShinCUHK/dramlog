...
## MVP 4단계: 익명 게시판 + 선택 로그인 (Supabase Auth 기반)
...
│   │       ├── auth.ts (인증 헬퍼 함수)
│   │       └── queries/
│   │           └── posts.ts (게시글 CRUD + 익명/로그인 분기)
│   └── components/
│       └── Header.svelte (로그인 상태 네비게이션)
```
...
- **같은 라우트에서 분기**: `/write`, `/posts/[id]/edit`, `/posts/[id]`에서 로그인 여부에 따라 UI/서버 로직을 분기합니다.
  - UI 예시: `$page.data.user`로 로그인 여부 확인 후 조건부 렌더링
  - 서버 예시: `getUser(cookies)`로 로그인 여부 확인 후 입력값/권한 검증 분기

- **글 작성** (`/write`):
  - 로그인 사용자: 작성자명은 닉네임(`user_metadata.nickname`, 없으면 email) 자동 적용, 비밀번호 입력 불필요, `user_id` 저장
  - 익명 사용자: 작성자 선택 입력(미입력 시 기본값), 비밀번호 필수, `edit_password_hash` 저장

- **글 수정/삭제** (`/posts/[id]/edit`, `/posts/[id]`):
  - 로그인 글(`user_id` 존재): 현재 사용자 ID와 `user_id` 비교 → 본인만 수정/삭제 가능
  - 익명 글(`user_id` 없음): 비밀번호 검증(`edit_password_hash`)으로 수정/삭제 가능

### 6. 쿼리 함수 업데이트

- `src/lib/server/supabase/queries/posts.ts`:
  - `createPost()`: (로그인) 비밀번호 없이 `user_id` 저장 / (익명) 비밀번호 해시 저장
  - `updatePost()`: (로그인) `user_id` 소유권 검증 / (익명) 비밀번호 검증
  - `deletePost()`: (로그인) `user_id` 소유권 검증 / (익명) 비밀번호 검증
