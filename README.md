...
3. SQL Editor에서 `supabase-schema.sql` 파일의 내용을 실행하여 테이블 생성 (`posts`, `post_tasting`, `comments`, `likes`, ...)
   - `pgcrypto` 확장 생성 포함 (hmac/uuid 사용)
   - 테이스팅 게시글 생성 RPC `create_tasting_post` 포함
...
- ✅ 검색 필터 (작성자/날짜/정렬/테이스팅)
- ✅ 페이지네이션 (12개/페이지) - `/posts`, `/my-posts`, `/search`에 적용
- ✅ WBTI 기획 문서 정리 - 자세히 보기: [wbti_기획_과정_정리.md](wbti_기획_과정_정리.md)
...
- 게시글/댓글/좋아요/알림 등 커뮤니티 핵심 기능 완료
- 익명/로그인 분기 및 RLS 정책 강화 완료
- 위스키 DB 연동 및 테이스팅(컬러/노즈/팔레트/피니시) 도입
